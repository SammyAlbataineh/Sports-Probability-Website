...
TEAM_GROUPS = {
    "Premier League": sorted([
        ...
        "Man City","Man United","Newcastle United","Nott'm Forest","Sunderland",
    ...
    "Serie A": sorted([
        ...
        "Hellas Verona","Milan","Juventus","Lazio","Lecce","AC Milan","Napoli",
        "Parma","Pisa","Roma","Sassuolo","Torino","Udinese"
    ]),
    "Bundesliga": sorted([
        "Augsburg","Union Berlin","Werder Bremen","Borussia Dortmund","Eintracht Frankfurt",
        "Freiburg","Hamburger SV","Heidenheim","Hoffenheim","Köln","RB Leipzig",
        "Bayer Leverkusen","Mainz 05","Borussia Mönchengladbach","Bayern Munich",
...
def build_match_result(match):
    try:
        ...
        result["Homeimg"] = TEAM_LOGOS.get(match["Home"], "")
        result["Awayimg"] = TEAM_LOGOS.get(match["Away"], "")
        result["League"] = LEAGUE_LABELS.get(match.get("League", ""), match.get("League", ""))
        result["Detail"] = match.get("Detail", "")
        result["Venue"] = match.get("Venue", "")
        if "Date" in match and match["Date"]:
            result["MatchDate"] = match["Date"].strftime("%a %d %b, %H:%M")
        else:
            result["MatchDate"] = ""
        return result
...
def home():
    ...
    results = [r for m in matches if (r := build_match_result(m)) is not None]
    logged_in = bool(session.get("name"))
    return render_template("home.html", matches=results, logged_in=logged_in, username=session.get("name", ""))


@app.route("/fixtures")
def fixtures():
    upcoming = todaysMatches.get_upcoming_matches(days=7)
    results = [r for m in upcoming if (r := build_match_result(m)) is not None]
    logged_in = bool(session.get("name"))
    return render_template("fixtures.html", matches=results, logged_in=logged_in, username=session.get("name", ""))


@app.route("/match/<int:match_id>")
def match_detail(match_id):
    all_matches = todaysMatches.get_upcoming_matches(days=7)
    for match in all_matches:
        result = build_match_result(match)
        if result and result["Id"] == match_id:
            logged_in = bool(session.get("name"))
            return render_template("match.html", match=result, logged_in=logged_in, username=session.get("name", ""))
    return "Match not found", 404


def _build_team_groups():
    try:
        from model import team_strengths
        league_map = {
            "EPL":        "Premier League",
            "LaLiga":     "La Liga",
            "SerieA":     "Serie A",
        ...
        groups = {v: [] for v in league_map.values()}
        for team, stats in team_strengths.items():
            league_label = league_map.get(stats["league"])
            if league_label:
                groups[league_label].append(team)
        for k in groups:
...
def custom():
    logged_in = bool(session.get("name"))
    groups = _build_team_groups()
    return render_template(
        "custom.html",
        team_groups=groups,
        logos_json=json.dumps(TEAM_LOGOS),
        logged_in=logged_in,
...
def predict():
    ...
    try:
        ...
        return jsonify({"error": f"Team not found in model: {e}"}), 400
    ...
    if request.method == "POST":
        ...
        try:
            conn = psycopg2.connect(
                database=db_info["db"], user=db_info["user"],
